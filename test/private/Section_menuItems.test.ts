...
describe("[private] Section.menuItems", () => {
  ...
  test("returns menuItems in the same order that the IDs are defined with createdAt and updatedAt", async () => {
    ...
    expect(data).toEqual({
      getMenu: {
        sections: [
          {
            menuItems: [existingMenuItem1, existingMenuItem2],
          },
        ],
      },
    });
